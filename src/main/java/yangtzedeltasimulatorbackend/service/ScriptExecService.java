    ...
    public JsonResult gdalClip(GDALClipDTO gdalClipDTO, String labTaskId, String userId) {
        try {
            ...
            if (re == 0) {
                ...
                dataObject.put("name", fileName);
                dataObject.put("type", "tif");
                dataObject.put("visualType", "tif");
                dataObject.put("size", String.valueOf(fileSize));
                dataObject.put("fileStoreName", fileName);
                dataObject.put("fileRelativePath", "/scriptOut/" + fileName);
                dataObject.put("fileWebAddress", "/store/scriptOut/" + fileName);
                dataObject.put("visualWebAddress", geoServerUrl);
                dataObject.put("visualizationBoolean", true);
                dataObject.put("publicBoolean", true);
...
//            String inputRasterPath = gdalClipDTO.getInputRasterPath();
            ...
            if(publish_1st.equals("true") && resultList.size() == inputRasterList.size()){
                ...
                JSONObject dataObject = new JSONObject();
                dataObject.put("id", IdUtil.objectId());
                dataObject.put("name", outFileName);
                dataObject.put("type", "tif");
                dataObject.put("visualType", "dataSet");
                dataObject.put("size", "0");
                dataObject.put("fileStoreName", outFileName + folderId);
                dataObject.put("fileRelativePath", "/scriptOut/" + outFileName + folderId);
                dataObject.put("fileWebAddress", "/store/scriptOut/" + outFileName + folderId);
                dataObject.put("visualWebAddress", tempGeoUrl);
                dataObject.put("visualizationBoolean", true);
                dataObject.put("publicBoolean", true);
                dataObject.put("source", "cloud");
                dataObject.put("dataSetList", resultList);
                dataList.add(dataObject);
                labTask.setDataList(dataList);
                labTaskDao.save(labTask);
