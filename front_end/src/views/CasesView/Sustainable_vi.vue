<template>
  <div class="container">
    ...
    <header id="header-panel">
      <h3 class="header-title">可持续发展目标评价专题展示系统</h3>
    </header>
    <h4
      style="position: absolute; top: 3%; left: 0.5%; color: white; z-index: 99"
    >
      体系选择:
    </h4>
    <el-select
      ...
      placeholder="require"
      class="selection"
      style="width: 250px; color: skyblue"
    >
      <el-option
    ...
    <div id="content-panel">
      ...
      <div class="card-panel row11">
        <div class="card">
          <div class="chart-header">
            <h4 class="chart-title multiChart-title">{{ require }}考核结果</h4>
          </div>
          <div id="map-show" class="map-panel"></div>
        </div>
      </div>
      <!-- .row12是右上角的六个柱状图模型容器 -->
      <div class="card-panel row12">
        <div class="multi-bar card">
          <div v-for="i in 6" class="chart-panel" :key="i">
            <div class="chart-header">
              <h4 class="chart-title multiChart-title">
                {{ barName3[i - 1] }}
              </h4>
            </div>
      ...
      <div class="card-panel row22">
        <div class="multi-pie card">
          <div id="pie-item-title" class="pie-item-title text-center">
            <h4 class="chart-title" style="text-align: center">
              各城市主要指标得分
            </h4>
          </div>
          <div
...
import {
  ...
  drawPie,
  drawBar,
} from "../../../public/case/sustainable/js/CreateChart";
  ...
  methods: {
      ...
      for (let i = 0; i < this.MongoDBDataOb.length; i++) {
        if (this.MongoDBDataOb[i].name == this.require) {
          for (let j = 0; j < this.MongoDBDataOb[i].data.length; j++) {
            if (
              this.barName2.indexOf(
                this.MongoDBDataOb[i].data[j][columnName]
              ) === -1
            ) {
    ...
    changeByTime() {
      ...
      if (this.barName2.length > 6) {
        this.timer2 = setInterval(() => {
          // 柱状图清空
          document
            .getElementById("bar-item1")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("bar-item2")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("bar-item3")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("bar-item4")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("bar-item5")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("bar-item6")
            .removeAttribute("_echarts_instance_");
          for (let i = 0; i <= 6; i++) {
            let name = "bar-item" + i;
      ...
      if (max > 5) {
        ...
        this.timer = setInterval(() => {
          // 饼图清空
          document
            .getElementById("pie-item1")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("pie-item2")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("pie-item3")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("pie-item4")
            .removeAttribute("_echarts_instance_");
          document
            .getElementById("pie-item5")
            .removeAttribute("_echarts_instance_");
          for (let i = 0; i <= 5; i++) {
            let name = "pie-item" + i;
            if (document.getElementById(name)) {
      ...
      axios.get(Global.SDGUrl + "/api/EcoData").then((response) => {
        ...
        for (let i = 0; i < this.tableNameOb.length; i++) {
          ...
          for (let j = 0; j < this.MongoDBDataOb.length; j++) {
            if (
              this.MongoDBDataOb[j].name == this.tableNameOb[i] &&
              this.MongoDBDataOb[j].data.length > 5
            ) {
    ...
    chartsRemove() {
      ...
      document
        ...
        .removeAttribute("_echarts_instance_");
      document
        .getElementById("pie-item-legend")
        .removeAttribute("_echarts_instance_");
      document
        .getElementById("bar-item1")
        .removeAttribute("_echarts_instance_");
...
.container {
  position: absolute;
  background: #050d3c url("../../../public/case/sustainable/pic/map_bg.png") 0 0/
    100% 100% no-repeat;
  width: 100%;
  height: 100%;
  min-width: 750px;
...
.card {
  ...
  border-style: solid;
  border-width: 10px;
  border-image: url("../../../public/case/sustainable/pic/chart-wrapper.png") 20 /
    1 / 0 repeat;
}
